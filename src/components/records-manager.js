...
export function recordsOfHashTag(hashTag, db){
    if(hashTag === "#noTag"){
        return {videos: [],
                featuredURLs: [],
                mathObjectLinks: [],
    ...
    let matches =  {
        videos: videosOfHashTag(hashTag, db),
        featuredURLs: featuredURLsOfHashTag(hashTag, db),
        mathObjectLinks: mathObjectLinksOfHashTag(hashTag, db),
        snippets: snippetsOfHashTag(hashTag, db)
    }
    return matches
}

function videosOfHashTag(hashTag, db){ 
    let vids = db.videos
...
function featuredURLsOfHashTag(hashTag, db){
    let furls = db.featuredURLs
    return furls.filter( (el) => {
          return el.hashTags.includes(hashTag)
         });
}

function mathObjectLinksOfHashTag(hashTag, db){
    let mols = db.mathObjectLinks
...
export function snippetsOfHashTag(hashTag, db){ 
    let snips = db.snippets
