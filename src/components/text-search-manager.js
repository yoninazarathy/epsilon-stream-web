...
export function hashTagOfString(searchString, htd) {
...
export function autoCompleteForString(s, hashTagDict) {
    let sLow = s.toLowerCase()
    let filtered = Object.keys(hashTagDict).filter((x)=>{return x.toLowerCase().includes(sLow)}) 
